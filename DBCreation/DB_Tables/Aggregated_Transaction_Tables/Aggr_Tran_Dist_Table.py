...
from PhonePeDataExtract.Data_Extractions.Aggregated_Data_Extracts.Aggregated_Trans_Data_Extract.Aggregated_State_Trans_Data_Extract import Aggr_State_Tran_Func
def Aggr_Tran_Dist_Table():
   ...
   # cursorObject.execute(aggr_tran_india)
    aggr_tan="""CREATE TABLE IF NOT EXISTS Aggr_Tran_Dist (
                            STATE VARCHAR(255),
                            YEAR YEAR(4),
                            QUATER INT,
    ...
    z = Aggr_State_Tran_Func()
    sql_alchamey=Sql_Alchamey_Helper()
    # Step 3: Convert the Pandas DataFrame to a format for MySQL table insertion
    z.to_sql('Aggr_Tran_Dist', con=sql_alchamey, if_exists='append', index=False)
x=Aggr_Tran_Dist_Table()
