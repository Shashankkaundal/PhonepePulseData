...
from PhonePeDataExtract.Data_Extractions.Aggregated_Data_Extracts.Aggregated_Insurance_Data_Extracts.Aggregated_India_Ins_Data_Extract import Aggregated_India_Ins_Func
def Aggr_Ins_India_Table():
   ...
   # cursorObject.execute(aggr_tran_india)
    aggr_tan="""CREATE TABLE IF NOT EXISTS Aggr_Ins_State (
                            COUNTRY VARCHAR(255),
                            YEAR YEAR(4),
                            QUATER INT,
    ...
    z = Aggregated_India_Ins_Func()
    sql_alchamey=Sql_Alchamey_Helper()
    # Step 3: Convert the Pandas DataFrame to a format for MySQL table insertion
    z.to_sql('Aggr_Ins_State', con=sql_alchamey, if_exists='append', index=False)

x=Aggr_Ins_India_Table()
