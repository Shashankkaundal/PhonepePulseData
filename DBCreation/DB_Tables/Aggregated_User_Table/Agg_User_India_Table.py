...
from PhonePeDataExtract.Data_Extractions.Aggregated_Data_Extracts.Aggregated_User_Data_Extract.Aggregated_India_User_Data_Extract import Aggregated_India_User_Func
def Aggr_User_India_Table():
   ...
   # cursorObject.execute(aggr_tran_india)
    aggr_tan="""CREATE TABLE IF NOT EXISTS Aggr_User_State (
                            COUNTRY VARCHAR(255),
                            YEAR YEAR(4),
                            QUATER INT,
                            BRAND VARCHAR(255),
    ...
    z = Aggregated_India_User_Func()
    sql_alchamey=Sql_Alchamey_Helper()
    # Step 3: Convert the Pandas DataFrame to a format for MySQL table insertion
    z.to_sql('Aggr_User_State', con=sql_alchamey, if_exists='append', index=False)

x=Aggr_User_India_Table()
