...
def Hover_India_Tran_Func():
    x=Local_Path_Func()
    path= x+"/map/transaction/hover/country/india/"
...
#print(clm)
    #dropping columns
    Hover_Trans_India=pd.DataFrame(clm)
    to_drop = ['Transaction_amount',
                'Transaction_type',
                'Quater']
    Hover_Trans_India.drop(to_drop, inplace=True, axis=1)


    #replacing and capitalising the states as per requirement
    Hover_Trans_India.replace('dadra & nagar haveli & daman & diu', 'Dadra and Nagar Haveli and Daman and Diu')
    Hover_Trans_India.replace('andaman & nicobar islands', 'Andaman & Nicobar')
    Hover_Trans_India.State_Name = Hover_Trans_India.State_Name.str.title()
    ##Taking Mean of all transaction by states for all the years
    avg_transactions_by_state = Hover_Trans_India.groupby(['State_Name','Year'], as_index=False)
